 ...
 * 
 * Implements the LlmProvider interface for Z AI (ZhipuAI / compatible API).
 * Uses OpenAI-compatible API format for flexibility.
 * Can be configured for any OpenAI-compatible endpoint.
 */
export class ZAiProvider implements LlmProvider {
  readonly providerName = "z_ai";

  // Default base URL - can be overridden via env if needed
  private readonly baseUrl: string = process.env.Z_AI_BASE_URL || "https://open.bigmodel.cn/api/paas/v4";

  isEnabled(): boolean {
    return Boolean(env.Z_AI_API_KEY);
  }
  ...
  private async generate(
    ...
    if (!env.Z_AI_API_KEY) return null;

    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), env.Z_AI_TIMEOUT_MS);
    ...
    try {
      ...
      if (response.status === 429) {
        logger.warn({ status: 429, provider: "z_ai" }, "Z AI rate limited");
        return null;
      }

      if (!response.ok) {
        logger.warn({ status: response.status, provider: "z_ai" }, "Z AI request failed");
        return null;
      }
      ...
      logger.warn({ err: error, provider: "z_ai" }, "Z AI unavailable");
      return null;
    } finally {
      clearTimeout(timer);
    }
