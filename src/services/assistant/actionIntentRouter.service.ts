...
const TIME_PHRASE =
  /\b(?:at\s+)?((?:1[0-2]|0?[1-9])(?::[0-5]\d)?\s*(?:am|pm)|(?:[01]?\d|2[0-3]):[0-5]\d)\b/i;
...
export function extractActionEntities(original: string, normalized: string): ExtractedActionEntities {
  ...
  const employeeMatch =
    original.match(/\b(?:with|assign(?:ed)?(?:\s+to)?)\s+([A-Z][a-zA-Z.'-]{1,40})\b/) ||
  ...
  const projectMatch = original.match(/\b(?:called|named)\s+([A-Za-z0-9][A-Za-z0-9 &.'-]{1,60}?)\s*$/i) ||
  ...
  if (createTask?.[1]) {
    ...
    title = title.replace(/^\bto\b\s+/i, "").replace(/\bfor\s+[A-Za-z].*$/i, "").trim();
  ...
  if (!entities.title && entities.employeeName && /schedule|meeting/.test(normalized)) {
...
async function resolveNamedList(
  ...
  if (items.length > 1) {
    return {
      clarification: {
        field,
        question: `I found ${items.length} ${label} matching ${name}. Which one do you mean?`,
...
export async function resolveActionEntities(
  ...
  if ((extracted.taskTitle || options.needTask) && intent !== "CREATE_TASK") {
    lookups.push(
      (async () => {
