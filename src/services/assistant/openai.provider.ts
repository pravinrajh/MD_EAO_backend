  ...
  private async generate(
    ...
    try {
      ...
      if (response.status === 429) {
        logger.warn({ status: 429, provider: "openai" }, "OpenAI rate limited");
        return null;
      }

      if (!response.ok) {
        logger.warn({ status: response.status, provider: "openai" }, "OpenAI request failed");
        return null;
      }
      ...
      logger.warn({ err: error, provider: "openai" }, "OpenAI unavailable");
      return null;
    } finally {
      clearTimeout(timer);
    }
