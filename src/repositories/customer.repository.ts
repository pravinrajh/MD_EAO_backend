import mongoose, { type ClientSession, type FilterQuery } from "mongoose";
import { Customer, type CustomerDocument } from "../models/Customer";
import { CUSTOMER_SAFE_FIELDS, type CustomerStatus } from "../utils/constants";
import { escapeRegex } from "../utils/pagination";

export type CustomerSortField = "createdAt" | "name" | "companyName" | "status";

export type CustomerListFilters = {
  search?: string;
  status?: CustomerStatus;
  assignedTo?: string;
  industry?: string;
  location?: string;
  scope?: FilterQuery<CustomerDocument>;
  skip: number;
  limit: number;
  sortBy: CustomerSortField;
  sortOrder: "asc" | "desc";
};

const CUSTOMER_UPDATE_FIELDS = [
  "name",
  "companyName",
  "email",
  "phone",
  "alternatePhone",
  "industry",
  "location",
  "address",
  "taxIdentifier",
  "assignedTo",
  "sourceLeadId",
  "status",
  "notes",
  "emailNormalized",
  "phoneNormalized",
  "companyNameNormalized",
  "isDeleted",
  "deletedAt",
  "deletedBy",
] as const;

function pickUpdate(input: Record<string, unknown>): Record<string, unknown> {
  const update: Record<string, unknown> = {};
  for (const field of CUSTOMER_UPDATE_FIELDS) {
    if (input[field] !== undefined) update[field] = input[field];
  }
  return update;
}

function toObjectId(value: unknown) {
  if (typeof value === "string") return new mongoose.Types.ObjectId(value);
  return value;
}

function stringifyIds(record: Record<string, unknown>) {
  record.id = String(record._id ?? record.id);
  if (record.assignedTo) record.assignedTo = String(record.assignedTo);
  if (record.createdBy) record.createdBy = String(record.createdBy);
  if (record.sourceLeadId) record.sourceLeadId = String(record.sourceLeadId);
  if (record.deletedBy) record.deletedBy = String(record.deletedBy);
  delete record._id;
  delete record.__v;
  delete record.emailNormalized;
  delete record.phoneNormalized;
  delete record.companyNameNormalized;
  return record;
}

export function toPublicCustomer(customer: CustomerDocument | Record<string, unknown>) {
  const record =
    typeof (customer as CustomerDocument).toJSON === "function"
      ? ((customer as CustomerDocument).toJSON() as Record<string, unknown>)
      : stringifyIds({ ...(customer as Record<string, unknown>) });
  if (typeof (customer as CustomerDocument).toJSON === "function") stringifyIds(record);
  return record;
}

function castScope(scope: FilterQuery<CustomerDocument> | undefined): FilterQuery<CustomerDocument> {
  if (!scope) return {};
  if (!Array.isArray(scope.$or)) return scope;
  return {
    $or: scope.$or.map((clause) => {
      const next: FilterQuery<CustomerDocument> = { ...clause };
      if (typeof next.createdBy === "string") next.createdBy = toObjectId(next.createdBy);
      const assigned = next.assignedTo as { $in?: unknown[] } | string | undefined;
      if (typeof assigned === "string") next.assignedTo = toObjectId(assigned);
      else if (assigned && Array.isArray(assigned.$in)) {
        next.assignedTo = { $in: assigned.$in.map((id) => toObjectId(id)) };
      }
      return next;
    }),
  };
}

function buildFilter(filters: CustomerListFilters): FilterQuery<CustomerDocument> {
  const query: FilterQuery<CustomerDocument> = { isDeleted: false };
  const and: FilterQuery<CustomerDocument>[] = [];

  if (filters.scope) and.push(castScope(filters.scope));
  if (filters.status) query.status = filters.status;
  if (filters.assignedTo) query.assignedTo = filters.assignedTo;
  if (filters.industry) query.industry = filters.industry;
  if (filters.location) query.location = filters.location;

  if (filters.search) {
    const term = escapeRegex(filters.search);
    and.push({
      $or: [
        { name: { $regex: term, $options: "i" } },
        { companyName: { $regex: term, $options: "i" } },
        { location: { $regex: term, $options: "i" } },
        { email: { $regex: `^${term}`, $options: "i" } },
        { phone: { $regex: term } },
        { customerId: { $regex: `^${term}`, $options: "i" } },
      ],
    });
  }

  if (and.length > 0) query.$and = and;
  return query;
}

export const customerRepository = {
  findById(id: string, session?: ClientSession | null) {
    const query = Customer.findById(id).select(CUSTOMER_SAFE_FIELDS).lean();
    if (session) query.session(session);
    return query;
  },

  findBySourceLeadId(leadId: string, session?: ClientSession | null) {
    const query = Customer.findOne({ sourceLeadId: leadId, isDeleted: false }).select(CUSTOMER_SAFE_FIELDS).lean();
    if (session) query.session(session);
    return query;
  },

  findSummariesByIds(ids: string[]) {
    if (ids.length === 0) return Promise.resolve([]);
    return Customer.find({ _id: { $in: ids }, isDeleted: false }).select("name companyName customerId status").lean();
  },

  create(data: Record<string, unknown>, session?: ClientSession | null) {
    if (session) return Customer.create([data], { session }).then((docs) => docs[0]);
    return Customer.create(data);
  },

  async list(filters: CustomerListFilters) {
    const query = buildFilter(filters);
    const sort: Record<string, 1 | -1> = { [filters.sortBy]: filters.sortOrder === "asc" ? 1 : -1 };
    const [items, total] = await Promise.all([
      Customer.find(query).select(CUSTOMER_SAFE_FIELDS).sort(sort).skip(filters.skip).limit(filters.limit).lean(),
      Customer.countDocuments(query),
    ]);
    return { items: items.map((item) => toPublicCustomer(item)), total };
  },

  updateById(id: string, input: Record<string, unknown>, session?: ClientSession | null) {
    return Customer.findByIdAndUpdate(id, { $set: pickUpdate(input) }, {
      new: true,
      runValidators: true,
      ...(session ? { session } : {}),
    }).select(CUSTOMER_SAFE_FIELDS);
  },

  findDuplicate(input: {
    emailNormalized?: string;
    phoneNormalized?: string;
    companyNameNormalized?: string;
    excludeId?: string;
    session?: ClientSession | null;
  }) {
    const or: FilterQuery<CustomerDocument>[] = [];
    if (input.emailNormalized) or.push({ emailNormalized: input.emailNormalized });
    if (input.phoneNormalized) or.push({ phoneNormalized: input.phoneNormalized });
    if (input.companyNameNormalized) or.push({ companyNameNormalized: input.companyNameNormalized });
    if (or.length === 0) return Promise.resolve(null);

    const query: FilterQuery<CustomerDocument> = { isDeleted: false, $or: or };
    if (input.excludeId) query._id = { $ne: toObjectId(input.excludeId) };
    const finder = Customer.findOne(query).select(CUSTOMER_SAFE_FIELDS).lean();
    if (input.session) finder.session(input.session);
    return finder;
  },

  async counts(scope: FilterQuery<CustomerDocument> | undefined) {
    const match: FilterQuery<CustomerDocument> = { isDeleted: false, ...castScope(scope) };
    const [facet] = await Customer.aggregate<{
      total: { n: number }[];
      byStatus: { _id: CustomerStatus; n: number }[];
    }>([
      { $match: match },
      {
        $facet: {
          total: [{ $count: "n" }],
          byStatus: [{ $group: { _id: "$status", n: { $sum: 1 } } }],
        },
      },
    ]);
    const byStatus = Object.fromEntries((facet?.byStatus ?? []).map((row) => [row._id, row.n]));
    return {
      total: facet?.total[0]?.n ?? 0,
      active: byStatus.ACTIVE ?? 0,
      inactive: byStatus.INACTIVE ?? 0,
      blocked: byStatus.BLOCKED ?? 0,
    };
  },

  toPublic: toPublicCustomer,
};
